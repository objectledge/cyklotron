 ...
 * @version $Id: CreateDocuments.java,v 1.1 2005-01-24 04:34:32 pablo Exp $
 */
    ...
    extends BaseAddEditNodeAction
{
        ...
        throws ProcessingException
    {
		...
		try
		{
            ...
            {
                String docName = name + "."+ (c+1);
                String docTitle = title + "."+ (c+1);
                NavigationNodeResource child =
                    structureService.addDocumentNode(docName, docTitle,node, subject);
                child.setSequence(c);
                setValidity(data, child);
                child.setStyle(style);
                child.setPriority(1);
                structureService.updateNode(child, docName, subject);
                if(structureService.isWorkflowEnabled())
                {
                    Permission permission = coralSession.getSecurity().getUniquePermission("cms.structure.modify_own");
                    if(subject.hasPermission(node,permission))
                    {
                        structureService.enterState(child, "taken", subject);
                    }
                    else
                    {
                        structureService.enterState(child, "new", subject);
                    }
                }
            }                
        }
