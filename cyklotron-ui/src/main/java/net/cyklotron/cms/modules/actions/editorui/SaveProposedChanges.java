        ...
        throws ProcessingException
    {
        ...
        try
        {
            ...
            {                
                ...
                if(showUnclassified)
                {
                    showUnclassified = structureService.showUnclassifiedInSite(cmsData.getSite());
                }
                // remove positiveCategory permanently if document has been modified and site uses showUnclassified 
                // this hit is done only for editorui users !!! 
                Resource positiveCategory = structureService.getPositiveCategory();
                boolean accept_doc_proposal = parameters.getBoolean("accept_doc_proposal", false);
                if(accept_doc_proposal && showUnclassified && positiveCategory != null)
                {
                    Relation relation = categoryService.getResourcesRelation(coralSession);
                    RelationModification modification = new RelationModification();
