 ...
 * @version $Id: AdvancedSearchMethod.java,v 1.1 2005-01-12 20:44:40 pablo Exp $
 */
public class AdvancedSearchMethod extends PageableResultsSearchMethod
{
    ...
    throws Exception
    {
        ...
        String qAnd = parameters.get("q_and").asString("");
        if(qAnd.length() > 0)
        {
        ...
        String qExpr = parameters.get("q_expr").asString("");
        if(qExpr.length() > 0)
        {
            BooleanClause clause =            
        ...
        String qOr = parameters.get("q_or").asString("");
        if(qOr.length() > 0)
        {
            BooleanClause clause =            
        ...
        String qNot = parameters.get("q_not").asString("");
        if(qNot.length() > 0)
        {
            BooleanClause clause =            
        ...
        String qTime = parameters.get("q_time").asString("all");
        ...
        String vTime = parameters.get("v_time").asString("all");
